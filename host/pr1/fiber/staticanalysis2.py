...
from typing import (Any, Generator, Generic, Literal, Mapping, Optional,
                    Protocol, Self, Sequence, TypeVar, cast)
...
InnerType = Any # ClassRef['InnerType'] | TypeVarDef
OuterType = Any # ClassRef['InnerType']
...
def accept_type_as_instantiable(input_type, *, type_variables: Optional[list[TypeVarDef]] = None):
...
def parse_type(node: ast.expr, /, variables: Variables, context: StaticAnalysisContext, *, instantiate_types: bool = True, type_variables = list()) -> tuple[StaticAnalysisAnalysis, InnerType]:
  match node:
    case ast.Attribute(obj, attr=attr_name, ctx=ast.Load()):
      analysis, obj_type = parse_type(obj, variables, context)

      assert isinstance(obj_type, Instance)

      attr = obj_type.cls.cls.instance_attrs.get(attr_name)

      return analysis, resolve_type_variables(attr, type_values={
        type_variable: type_arg for type_variable, type_arg in zip(obj_type.cls.cls.type_variables, obj_type.cls.type_args)
      })
    ...
    case ast.Call(func, args, keywords):
      analysis, func_type = parse_type(func, variables, context)

      if func_type == TypeVarClassDef:
        match args, keywords:
        ...
        args = analysis.add_sequence([parse_type(arg, variables, context) for arg in args])
        kwargs = analysis.add_mapping({ keyword.arg: parse_type(keyword.value, variables, context) for keyword in keywords if keyword.arg })

        overload = find_overload(instantiable_type.cls.instance_attrs['__init__'], args=args, kwargs=kwargs)
    ...
    case ast.Name(id=name, ctx=ast.Load()):
      ...
      match value:
        ...
        case ClassDef() if instantiate_types:
          return StaticAnalysisAnalysis(), value
    ...
    case ast.Subscript(value=ast.Name(id=name, ctx=ast.Load()), slice=subscript, ctx=ast.Load()):
      ...
      if subscript_value == GenericClassDef:
        ...
        return StaticAnalysisAnalysis(), GenericClassDefWithGenerics(type_vars)


      if not isinstance(subscript_value, ClassDef):
      ...
      for arg in expr_args:
        arg_raw_type = analysis.add(parse_type(arg, variables, context))
        arg_type = analysis.add(accept_type_as_instantiable(arg_raw_type, type_variables=type_variables))
      ...
      return analysis, InstantiableClassDef(subscript_value, type_args=type_args)
  ...
  for module_statement in module.body:
    match module_statement:
      case ast.AnnAssign(target=ast.Name(id=name, ctx=ast.Store()), annotation=ann, value=None, simple=1):
        ...
        ann_type = analysis.add(parse_type(ann, variables | values, context))
        values[name] = Instance(analysis.add(accept_type_as_instantiable(ann_type)))
      ...
      case ast.ClassDef(name=class_name, bases=class_bases, body=class_body):
        ...
        for class_base in class_bases:
          base_type = analysis.add(parse_type(class_base, variables | values, context, instantiate_types=False))

          if isinstance(base_type, GenericClassDefWithGenerics) and (not type_variables_set):
            cls.type_variables = base_type.typevars
        ...
        for class_statement in class_body:
          match class_statement:
            ...
            case ast.AnnAssign(target=ast.Attribute(attr=attr_name, value=ast.Name(id='self')), annotation=attr_ann, simple=0):
              ...
              ann_type = analysis.add(parse_type(attr_ann, variables | values, context, type_variables=cls.type_variables))
              cls.instance_attrs[attr_name] = Instance(analysis.add(accept_type_as_instantiable(ann_type, type_variables=cls.type_variables)))
        ...
        if not init_func.overloads:
          init_func.overloads.append(FuncOverloadDef(
            ...
            args_posonly=list(),
            default_count=0,
            return_type=Instance(InstantiableClassDef(NoneType))
          ))
...
def resolve_type_variables(input_type, type_values):
  ...
  match input_type:
    ...
    case Instance(cls):
      return Instance(resolve_type_variables(cls, type_values))
...
def find_overload(func: FuncDef, /, args: list[ClassRef], kwargs: dict[str, ClassRef], *, known_type_variables: Optional[dict[TypeVarDef, Any]] = None):
...
PreludeVariables = CoreVariables
...
""", PreludeVariables)
  ...
  document = Document.text("~~~ A[int]().bar.foo ~~~")
  context = StaticAnalysisContext(
  ...
  analysis, result = parse_type(root.body, user_variables, context)
